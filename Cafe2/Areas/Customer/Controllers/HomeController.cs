...
namespace Cafe2.Areas.Customer.Controllers
{
    [Area("Customer")]
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Contact([Bind("Id,Name,Email,Telefon,Mesaj")] Contact contact)
        {
            if (ModelState.IsValid)
            {
                ...
                _toast.AddSuccessToastMessage("Thank you. Your message has been successfully delivered...");
        ...
        public async Task<IActionResult> Blog(Blog blog)
        {
            if (ModelState.IsValid)
            {
                ...
                _db.Add(blog);
                await _db.SaveChangesAsync();
                _toast.AddSuccessToastMessage("Thank you. Your comment has been submitted. When your comment is approved, you can see it on the Blog Page.");
                return RedirectToAction(nameof(Index));
            }
            return View(blog);
        }
        ...
        public async Task<IActionResult> Rezervasyon([Bind("Id,Name,Email,TelefonNo,Sayi,Saat,Tarih")] Rezervasyon rezervasyon)
        {
            if (ModelState.IsValid)
            {
                _db.Add(rezervasyon);
                await _db.SaveChangesAsync();
                _toast.AddSuccessToastMessage("Thank you. Reservation is completed...");
                return RedirectToAction(nameof(Index));
            }
